...
(function(){
  ...
  function computePlots() {
    ...
    const fieldTrace = {
      ...
      marker: {
        ...
        line: { width: 0 },
        colorbar: {
          title: { text: "Fraction of maximal damage" },
          thickness: 16, len: 0.82, tickformat: ".2f",
          x: 1.05, xanchor: "left"
        }
      },
      hovertemplate:
  ...
  function buildRally(fractions, rallySize, stock) {
    ...
    if (aTarget < 0) { cTarget = Math.max(cMin, rallySize - iTarget); aTarget = rallySize - iTarget - cTarget; }
    ...
    if (deficit > 0) { let give = Math.min(deficit, Math.max(0, stock.arc - a)); a += give; deficit -= give; }
    if (deficit > 0) { let give = Math.min(deficit, Math.max(0, stock.cav - c)); c += give; deficit -= give; }
    if (deficit > 0) { let canInf = Math.min(iMax, stock.inf) - i; let give = Math.min(deficit, Math.max(0, canInf)); i += give; deficit -= give; }

    let surplus = (i+c+a) - rallySize;
    if (surplus > 0) { let take = Math.min(surplus, a); a -= take; surplus -= take; }
    if (surplus > 0) { let minC = Math.min(cMin, c); let take = Math.min(surplus, c - minC); c -= take; surplus -= take; }
    if (surplus > 0) { let minI = Math.min(iMin, i); let take = Math.min(surplus, i - minI); i -= take; surplus -= take; }
  ...
  function buildOptionAFormations(stock, formations, cap) {
    ...
    for (let i=0; i<n; i++) {
      ...
      if (free > 0) {
        ...
        infAlloc[i] += giveInf; stock.inf -= giveInf; free -= giveInf;
      }
      if (free > 0) {
  ...
  function onOptimize() {
    ...
    let html = `<table><thead>
      <tr><th>Type</th><th>Inf</th><th>Cav</th><th>Arc</th><th>Total</th></tr>
    </thead><tbody>`;
    if (rallySize > 0) {
